...
class CoreWriter:
    ...
    def write(self, profile: RepoProfile, section: str, instructions: str, **kwargs) -> str:
        ...
        # 1. Retrieve Context specific to this section
        context = ""
        try:
            store = get_vector_store(profile.name)
            retriever = get_retriever(store)
            # Query based on the section name
            docs = retriever.invoke(f"{section} setup installation usage examples")
