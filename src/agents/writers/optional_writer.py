...
class OptionalWriter:
    def __init__(self, model_name: str = "gpt-5.1"):
        ...
        prompt_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), "src/prompts/writers/optional_writer.txt")
        with open(prompt_path, "r") as f:
            self.prompt_template = f.read()

    def write(self, profile: RepoProfile, section_title: str, instructions: str = "") -> str:
        print(f"[{profile.name}] 3.4 Optional Writer: {section_title}...")
        
        context = ""
        try:
            store = get_vector_store(profile.name)
            retriever = get_retriever(store)
            # Use instructions to guide retrieval
            query = f"{section_title} " + (instructions if instructions else "")
            docs = retriever.invoke(query)
            context = "\n".join([d.page_content[:500] for d in docs])
        except Exception:
            pass

        # Truncate profile for Optional Writer
        ...
        if len(profile_copy.usage_snippets) > 5:
            profile_copy.usage_snippets = profile_copy.usage_snippets[:5]
